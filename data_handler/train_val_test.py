...
def reduce_t_images(data, reduction_factor=3):
    t_images = [item for item in data if '_t' in item[0]]
    non_t_images = [item for item in data if '_t' not in item[0]]
    print(f"Original '_t' images count: {len(t_images)}")
    reduced_t_images = random.sample(t_images, len(t_images) // reduction_factor)
    print(f"Reduced '_t' images count: {len(reduced_t_images)}")
    return reduced_t_images + non_t_images
